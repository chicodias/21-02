...
Este estudo usou cadeias de nascimento e morte em ambientes aleatórios para modelar tais populações, e buscou compreender se a dispersão aleatória é uma boa estratégia para a sobrevivência da população. 
...
tibble(y = getStates(dat), x = getTimes(dat), lambda=Lambda, x0 = initstate)
}
...
## Conclusão
  ...
  - O **Teorema 2** mostra que sem a dispersão o ambiente aleatório não auxilia na sobrevivência.
  
- Se olharmos para uma cadeia de nascimento e morte com taxa de nascimento $E(\Lambda)$ e taxa de morte 1 o processo sobrevive, assim como no modelo de dispersão.
