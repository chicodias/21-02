...
Temos $\theta_A \sim N(900, 20)$ e $\theta_B \sim N(900, 80)$. Assim:

```{r fig1, out.width = '80%',  fig.align = "center", echo = FALSE, warning = FALSE}
...
$$\mu_1 = \frac{\tau_0^2 \mu_0 + \sigma^{-2}\bar{x}}{\tau_0^{-2} + \sigma^{-2}},\ \ \text{e}\ \ \tau_1^{-2}= \tau_0^{-2}+ \sigma^{-2}$$

Assim, para 100 observações temos:

$$\theta_A|\tilde{x}_{100} \sim N(884.314, 15.3846), \ \ \theta_B|\tilde{x}_{100} \sim N(883.7272, 15.9601) $$

Enquanto que para uma única observação, a posteriori é:

$$\theta_A|\tilde{x}_{1} \sim N(898.4, 320), \ \ \theta_B|\tilde{x}_{1} \sim N(893.6, 1280) $$


```{r fig2, out.width = '80%',  fig.align = "center", echo = FALSE, warning = FALSE}
a %>%
  ggplot(aes(x = theta)) +
...
- para o físico A: precisão($\theta$) passou de $\tau_0^{-2} = 0.0025$ a $\tau_1^{-2} = 0.0042$ (aumento de 70%)
- para o físico B: precisão($\theta$) passou de $\tau_0^{-2} = 0.0001$ a $\tau_1^{-2} = 0.0039$ (aumento de 3900%)

Diminuições nas precisões da posteriori com 1 observação:

- para o físico A: precisão($\theta$) passou de $\tau_0^{-2} = 0.0025$ a $\tau_1^{-2} < 10^{-6}$.
- para o físico B: precisão($\theta$) passou de $\tau_0^{-2} = 0.0001$ a $\tau_1^{-2} <10^{-6}$.
...
tibble(      Fisico = c("A","B"),
             ...
             SD.pos.1 = c(320 ,1280)) %>% 
  knitr::kable(col.names = c ("Físico", "$\\mu_0$", "$\\mu_1(n = 100)$", "$\\mu_1(n = 1)$", "$\\tau_0$", "$\\tau_1(n = 100)$", "$\\tau_1(n = 1)$"))
...
Dessa forma, $X_A \sim N(900, 44.7213)$ e $X_B \sim N(900,89.4427)$.
