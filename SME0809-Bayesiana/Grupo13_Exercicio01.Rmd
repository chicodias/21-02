...
a <- rbind(norm(dados, tau0 = 20) %>% mutate(cor = "Fisico A"),
      norm(dados, tau0 = 80) %>% mutate(cor = "Fisico B"))
...
Como $X|\theta \sim N(\theta, 40^2)$ e $\theta \sim N(\mu_0,\tau_0^2)$ então $\theta|x \sim N(\mu_1, \tau_1)$, sendo

$$\mu_1 = \frac{\tau_0^2 \mu_0 + \sigma^{-2}x}{\tau_0^{-2} + \sigma^{-2}},\ \ \text{e}\ \ \tau_1^{-2}= \tau_0^{-2}+ \sigma^{-2}$$

Assim,

$$\theta_A \sim N(), \ \ \theta_B \sim N() $$
