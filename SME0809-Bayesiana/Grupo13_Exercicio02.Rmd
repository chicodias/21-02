...
\sum_{i=1}^n \left( y_i\log(\theta) -\theta \right)$$


```{r}
Ltheta <- function(theta,sample)(sum(sample)*(log(theta)) - length(sample)*theta)

theta <- 0:20


  a <- seq(1:10) %>% map_dfr( ~tibble( x = theta,
                                  y = dpois(theta, lambda = .x),
                                  L = Ltheta(theta, rpois(30, .x)),
                                  lambda = as.factor(.x)))
  
  a %>%  ggplot(aes(x = x)) + #geom_line(aes(y = y, color = lambda)) +
    geom_line(aes(y = L, color = lambda), lty=2) +
