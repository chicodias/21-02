...
A distribuição a *priori* de Jeffreys é dada por $\pi(\theta) \propto \sqrt{J(\theta)}$.
...
Antes de prosseguir, vamos relembrar alguns resultados:

Se $X_1 \sim \text{Gama}(\alpha,\beta)$ então
$$ f_{X_1}(x | \alpha,\beta) = \frac{\beta^\alpha x^{(\alpha - 1)} e^{-\beta x}}{\Gamma(\alpha)} \propto x^{(\alpha - 1)} e^{-\beta x}$$

Se $X_2 \sim \text{Gama-Inv}(\alpha,\beta)$ então
$$ f_{X_2}(x | \alpha,\beta) = \frac{\beta^\alpha x^{(\alpha - 1)} e^{-\beta / x}}{\Gamma(\alpha)} \propto x^{(\alpha - 1)} e^{-\beta/x}$$

Se $X_3 \sim \chi^2 (\nu)$ então
$$ f_{X_3}(x | \nu) = \frac{x^{(\nu/2) - 1} e^{- x/2}}{2^{\nu/2}\Gamma(\nu/2)} \propto x^{(\nu/2) - 1} e^{- x/2}$$

Note que, desprezadas as constantes não informativas, as três distribuições são da forma $x$ elevado a uma potência vezes a exponencial de $x$ vezes algo. Dessa forma, as três distribuições servem como conjulgada natural da Normal, em nosso caso. Para explicitar os parâmetros de cada uma delas, vamos substituir o $x$ por $\sigma$ e ver o que acontece:
