...
Chegam duas amostras de bico de pinguins, uma com $n = 2$, outra com $n = 15$. 


```{r tab1}
tibble(      Prioris = c("Priori 1","Priori 2","Priori 3", "Priori 4"),
  ...
  knitr::kable(digits = 2, caption = "Resumo das prioris dos quatro amigos")
```
...
a <- norm(samp = sample2,  sigma = sd(sample)+1, tau0 = 20, mu = 42) %>% mutate(Priori = "Cleiton") 
b <- norm(samp = sample2,  sigma = sd(sample), tau0 = 8, mu = 30) %>% mutate(Priori = "Eduarda")
c <- norm(samp = sample2,  sigma = sd(sample), tau0 = 14 , mu = 60) %>% mutate(Priori = "Larissa")
d <- norm(samp = sample2,  sigma = sd(sample), tau0 = 12, mu = 54) %>% mutate(Priori = "Robertinho")
...
```{r graf12, warning=FALSE}


a <- norm(samp = sample15,  sigma = sd(sample)+1, tau0 = 20, mu = 42) %>% mutate(Priori = "Cleiton") 
b <- norm(samp = sample15,  sigma = sd(sample), tau0 = 8, mu = 30) %>% mutate(Priori = "Eduarda")
c <- norm(samp = sample15,  sigma = sd(sample), tau0 = 14 , mu = 60) %>% mutate(Priori = "Larissa")
d <- norm(samp = sample15,  sigma = sd(sample), tau0 = 12, mu = 54) %>% mutate(Priori = "Robertinho")
...
Chegam duas amostras de bico de pinguins, uma com $n = 5$, outra com $n = 30$. 
...
b <- SigmaNorm(sample5,alpha = 1.5, beta = 1.5) %>% mutate(Priori = "Eduarda")
...
```{r graf22, warning = FALSE}
a <- SigmaNorm(sample30, alpha = 0.01, beta = 0.01) %>% mutate(Priori = "Cleiton") 
b <- SigmaNorm(sample30,alpha = 1.5, beta = 1.5) %>% mutate(Priori = "Eduarda")
