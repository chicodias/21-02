...
```
Outra versão dessa matriz de gráficos pode ser disposta abaixo, atribuindo cor a covariável Tamanho.

```{r message = F}
GGally::ggpairs(dados, columns = c(1:4,6), aes(colour = as_factor(Tamanho)))
```

Também podemos observar individualmente os gráficos de dispersão - na versão original e atribuindo cor a covariável Tamanho -, que comprovam e ilustram os resultados da nossa tabela de correlação, onde temos que as variáveis são pouco ou nada correlacionadas.

```{r}
dados %>% pivot_longer(!c(Tamanho, y)) %>%
  ggplot(aes(y = y, color = as_factor(Tamanho))) + geom_point(aes(x = value)) +
  facet_wrap(~name, scales = "free_x") + labs(x       = "Variáveis",
         y       = "Valores", 
...
Do mesmo modo, temos predisposto para visualização uma matriz de nossos histogramas.
...
```{r}
GGally::ggparcoord(dados) + labs(x       = "Variáveis",
         y       = "Valores", 
...
Equação de melhor modelo: $Y = \beta_2x_2 + \beta_5x_5$
