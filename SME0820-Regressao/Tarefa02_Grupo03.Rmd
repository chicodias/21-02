...
A partir dos gráficos de barras, podemos ver que nossas cinco covariáveis, apesar de serem quantitativas, assumem apenas dois valores. A única variável que assume mais valores do que isso é $y$, que apararenta ter uma distribuição quase uniforme.
...
```
 ...
 Em relação à diferença entre o EMV e o estimador de mínimos quadrados de beta é, basicamente, que, estando sob a suposição de erro normal, como normalmente assumimos em regressão linear, o estimador de mínimos quadrados e a máxima verossimilhança são os mesmos. Ou seja, minimizar o erro ao quadrado é equivalente a maximizar a probabilidade caso as seguintes condições sejam cumpridas:
...
Temos que:
$SQ_{res} = \sum_{i=1}^{n}(Y_i - \hat{Y})^2 = (\boldsymbol{y-\hat{y}})^T(\boldsymbol{y-\hat{y}}) = (\boldsymbol{y}-X\hat{\beta})^T(\boldsymbol{y}-X\hat{\beta}) = (\boldsymbol{y}^T-X^T\hat{\beta}^T)(\boldsymbol{y}-X\hat{\beta}) = \boldsymbol{y^Ty - Y^T}X\hat{\beta}-\hat{\beta}^TX^T \boldsymbol{y}+\hat{\beta}^TX^TX\hat{\beta} = \boldsymbol{y^Ty} - 2\hat{\beta}^TX^T\boldsymbol{y} + \hat{\beta}^TX^TX\hat{\beta}$
...
$SQ_{res}= \boldsymbol{y}^{T}\boldsymbol{y} - ((X^TX)^{-1}X^T\boldsymbol{y})^TX^T\boldsymbol{y} = \boldsymbol{y}^{T}\boldsymbol{y} - \boldsymbol{y}^TX(X^TX)^{-1}X^T\boldsymbol{y} = \boldsymbol{y}^{T}\boldsymbol{y} - \boldsymbol{y}^{T}H\boldsymbol{y}$
...
- Desenvolvendo $E(\boldsymbol{y}^{T}\boldsymbol{y})$:
  ...
  $E(\boldsymbol{y}^{T}\boldsymbol{y}) = E(\boldsymbol{y}^{T}I\boldsymbol{y}) = tr(I\sigma^2I)+ (X\beta)^TIX\beta = \sigma^2tr(I)+\beta^TX^TX\beta = n\sigma^2 + \beta^TX^TX\beta$
  
- Desenvolvendo $E(\boldsymbol{y}^{T}H\boldsymbol{y})$:
  $E(\boldsymbol{y}^{T}H\boldsymbol{y}) = tr(H\sigma^2I) + (X\beta)^TX(X^TX)^{-1}X^TX\beta = \sigma^2tr(H) + \beta^TX^TX(X^TX)^{-1}X^TX\beta = \sigma^2p + \beta^TX^TX\beta$
