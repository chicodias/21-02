...
X <- matrix(c(rep(1,n), dados$Pressao, dados$Temp, dados$FluxoCO2, dados$Umidade, dados$Tamanho), ncol = 6, nrow = n, byrow = FALSE)

Y <- matrix(dados$y, ncol = 1, nrow = n)
...
```{r}
dados %>% pivot_longer(cols = !Tamanho) %>% 
  ggplot() + geom_bar(aes(x = as_factor(value), fill = as_factor(Tamanho)),stat="count") +
  facet_wrap(~name, scales = "free_x")
```
