...
dados %>% 
  ggplot(aes(x = "", y = dados$y)) +
    ...
    xlab("") + ylab("Rendimento do motor (mi/L")
```
Pelo boxplot acima não fica claro qual seria a distribuição de y, porém, podemos ver que existem 4 outliers bem claros.Vamos identificar seus valores.

```{r}
dados %>%
  ggplot(aes( x=dados$y)) +
    geom_histogram(color = "black", fill = "lightblue") + # xlab("Rendimento") + ylab("Frequência") +
...
dados %>%
  ggplot(aes( x=dados$x1)) +
    geom_histogram(color = "black", fill = "lightgreen") + # xlab("Rendimento") + ylab("Frequência") +
...
```{r}
# Adicionando aqui pq os itens 3 e 4 dependem disso, é parte do código do monitor mesmo, só avisar se mudar os nomes das variáveis ;) - Monica
...
ggplot(dados, aes(x = res)) +
  ylab("Frequência") +
  geom_histogram(color = "black", fill = "#FF69B4", bins=n_res)+
...
```
  ...
  Resumo: a fazer
  Análise do histograma/resíduos: a fazer
...
```
  ...
  A COMPLETAR/MODIFICAR
  
  Para realizarmos nossos testes de hipóteses, primeiro precisamos estimar o parâmetro $\sigma^2$ do nosso modelo, uma vez que ele não é dado. Tal estimador não viesado é obtido através da soma do quadrado dos resíduos, definido pela variável *QMres*, de modo que:
...
```
  ...
  * Testagem se $\beta_1 = 0$:
  ...
  $\beta_1$ possuí distribuição Normal com média $\beta_1$ e variância $\sigma^2/S_{xx}$
...
```
  ...
  * Testagem se $\beta_0 = 0$:
  ...
  $\beta_0$ possuí distribuição Normal com média $\beta_0$ e variância $\sigma^2((\frac{1}{n})+\frac{\bar{X}}{S_{xx}}))$
...
```
  ...
  Em um processo semelhante à testagem de $\beta_1$, é dado que $\alpha$= 5%. Se H0 não for rejeitado, temos que $\beta_0$ é estatisticamente igual a zero. 
