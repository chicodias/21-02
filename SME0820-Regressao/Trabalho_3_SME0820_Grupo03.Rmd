...
```{r}
pander(summary(data), "Sumário do conjunto de dados")
```
...
Devido ao comportamento singular de nosso conjunto de dados observado anteriormente na análise exploratória, todos os valores da diagonal da matriz hat são iguais, o que torna o exemplo muito interessante do ponto de vista do aprendizado.
...
```{r}
nome_colunas <- c('Observação $i$', '$e_i = y_i - \\hat{y_i}$', "d_i", "r_i", "h_ij", "t_i")
tab <- tibble(i= 1:16,res, res.ext.st, res.int.st,h,res.padr)
pander(tab, col.names = nome_colunas, format = "markdown")
```
...
Não foi observado nenhum padrão nos plots residuais, o que nos fornece indícios gráficos de uma certa dispersão aleatória dos resíduos, com os escalonamentos aparentando ter contribuído para dimição da variância, como seria esperado.  


  
## 8. Transformações

*** Análise gráfica dos gráfico dos resíduos pelas covariáveis ***
```{r}
par(mfrow = c(1, 1))
plot(res ~ dados$Temp, ylab = "Residuos", xlab = "covariavel temp", main = "Residuos versus Temperatura")
```

```{r}
par(mfrow = c(1, 1))
plot(res ~ dados$Tamanho, ylab = "Residuos", xlab = "covariavel tamanho", main = "Residuos versus Tamanho")
```

Conforme constatado através das análises realizadas acima e nas questões anteriores, é fácil concluir que o ajuste do modelo de regressão linear simples com as variáveis na sua forma original é inadequado neste caso. 

A fim de solucionar os problemas, como não há uma relação bem definida entre as covariáveis, optaremos por uma transformação na variável resposta.

Um fato relevante para ser levado em conta antes da transformação, é que conforme observado na questão 3 e comprovado pelo teste de shapiro, os resíduos apresentam distribuição normal. No entando, foi concluído pelo teste de Breush-Paganque há heterocedasticidade nos dados; nossos dados não possuem um padrão, e a variância não é constante.

### Fazendo a tranformação na variável resposta
```{r}
mod_logy <- lm(log(y) ~ dados$Temp+dados$Tamanho , data = dados)
summary(mod_logy)
```
...
n <- length(dados$y)
Xmod_logy <- NULL
Xmod_logy <- matrix(c(rep(1,n),x2,x5), nrow=n, ncol=3 )
...
plot(res_mod_logy ~ y_estmod_logy, ylab = "Residuos", xlab = "Valores ajustados", main = "Res
iduos")
plot(res.padr_mod_logy ~ y_estmod_logy, ylab = "Residuos padronizados", xlab = "Valores ajust
ados", main = "Padronizados", ylim = c(-3, 3))
